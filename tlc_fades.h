...
int16_t currentValues[NUM_TLCS * 16];
...
int16_t tlc_getCurrentValue(TLC_CHANNEL_TYPE channel);
                    ...
                    int16_t endValue, uint32_t startMillis, uint32_t endMillis)
{
    ...
    currentValues[channel] = startValue;
...
uint8_t tlc_updateFades(uint32_t currentMillis)
{
    ...
    for (struct Tlc_Fade *p = tlc_fadeBuffer; p < end;){
        if (currentMillis >= p->endMillis) { // fade done
            ...
            currentValues[p->channel] = p->startValue + p->changeValue;
            needsUpdate = 1;
            tlc_removeFadeFromBuffer(p, --end);
            continue;
        } else {
            ...
            if (currentMillis >= startMillis) {
                ...
                currentValues[p->channel] = p->startValue + p->changeValue * (int32_t)(currentMillis - startMillis) / (int32_t)(p->endMillis - startMillis);
                needsUpdate = 1;
            }
        }
        p++;
